-- Surfaces birthday through get_public_profile() so the public page can
-- show it under the avatar (month/day only — the client never renders
-- the year, but the RPC returns the full date since that's the source
-- of truth and there's nothing sensitive about it living server-side).
...
begin
  ...
  select jsonb_build_object(
    ...
    'avatarEmoji', v_profile.avatar_emoji,
    'avatarEmojiBg', v_profile.avatar_emoji_bg,
    ...
    'managedProfiles', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'avatarEmojiBg', m.avatar_emoji_bg,
          'birthday', m.birthday
        )
